...
#---------- Execute on the robot-----------#
    ...
    p1=[0.558,-0.251,0.350,-90.0,-90.0,90.0]                   #Leftmost Ikura bowl
    ...
    p2=[0.558-(x*0.02),-0.192,0.185,-90.0,-90.0,90.0]          #ikura bowls
    p2_2=[0.558-(x*0.02),-0.291,0.185,-90.0,-90.0,90.0]        #ikura bowls
    ##p10=[0.558,-0.251,0.200-x*0.0015,-90.0,-90.0,90.0]         #ikura bwol midpoint
    p10=[0.558,-0.251,0.200-x*0.0030,-90.0,-90.0,90.0]         #ikura bwol midpoint
    p3=[0.450,0.004,0.350,-90.0,-90.0,90.0]                    #Center target bowl
    p4=[0.450,0.004,0.250,-90.0,-90.0,90.0]                    #Gari dropping point
    p7=[0.450,-0.045,0.240,-90.0,-90.0,90.0]                   #Zig zag
    p8=[0.450,0.035,0.240,-90.0,-90.0,90.0]                    #Zig zag
    p9=[0.396,0.004,0.250,0.0,-90.0,90.0]
    p11=[0.450,0.035,0.190,-90.0,-90.0,90.0]
    arm="right"
    h=0                                                          #gaari feed
    def ikura_pick(a,b):
    ...
    def ikura_pick_mid():
        success = plan_move(arm,[p1,p10],[1,1])
        motion_seq = get_motion()
        clear_motion()
        nxt.playMotion(motion_seq)
    ...
    def ikura_drop():
        controllerClient.setServoSpeed(15)
        controllerClient.controlGripper("bothServos_down", 4*degree_3,4*degree_3)
        time.sleep(1)

        controllerClient.setServoSpeed(30)
        #controllerClient.controlGripper("bothServos_up", (4*degree_3)-degree-10,(4*degree_3)-degree-10)
        controllerClient.controlGripper("bothServos_up", (4*degree_3)-degree,(4*degree_3)-degree)

    def gari_box():
        nxt.openHandToolLft()
        arm="left"

        p5=[0.507,0.180,0.250,-90.0,-90.0,90.0]             #gari top
        
        p6=[0.507-0.03*x,0.180,0.126,-90.0,-90.0,90.0]      #gari right

        success = plan_move(arm,[p5,p6],[1,1])
        motion_seq = get_motion()
        clear_motion()
        nxt.playMotion(motion_seq)
        ...
        success = plan_move(arm,[p5,p11],[1,1])
        motion_seq = get_motion()
        clear_motion()
        nxt.playMotion(motion_seq)
    ...
    def gari_box_2():
        ...
        p6=[0.507-0.03*x,0.240,0.126,-90.0,-90.0,90.0]      #gari left
    ...
    def gari_pick1():
        ...
        p6=[0.461,0.227,0.170-x*0.003,-90.0,-90.0,90.0]
        success = plan_move(arm,[p5,p6],[1,2])
        motion_seq = get_motion()
        clear_motion()
        nxt.playMotion(motion_seq)
        ...
        success = plan_move(arm,[p5,p4],[1,2])
        motion_seq = get_motion()
        clear_motion()
        nxt.playMotion(motion_seq)
