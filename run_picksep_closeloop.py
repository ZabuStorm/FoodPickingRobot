...
def capture(method="fge"): 
    ...
    if point_array is not None:
        ...
        crop = crop_roi(img_blur, cfgdata, bounding_size=40)
...
def regrasp(xyz_s, xyz_e):
    ...
    if PLAY:
        ...
        nxt.playMotion(motion_seq, wait=False)
        _, out_cont = monitoring(max_tm=np.sum(motion_seq[:,0]))

        _, out2 = monitoring(max_tm=1)
...
def swing(pose, j, repeat=1, bilateral=False):
    print("* * * * * * * * * *")
    print("*      Swing      *")
    print("* * * * * * * * * *")
    p = pose[:3]
    ...
    success = plan_swing(arm, p, q, p_end, j, tm, repeat, bilateral=bilateral)
    motion_seq = get_motion()
...
def transport(pose):
    ...
    if PLAY: 
        ...
        # jnt, out = monitoring(max_tm=np.sum(motion_seq[:,0]), f_thld=k_stop+0.6)
        jnt, out = monitoring(max_tm=np.sum(motion_seq[:,0]), f_thld=k_stop)
        # fs.plot(out, filter=True)
        clear_motion(jnt)
...
while (i < max_picking):
    ...
    pick(eef_pose)
    # eef_pose = [0.500,-0.054,0.300,90,-90,-90]

    # success = plan_move(arm, [eef_pose], [5], init=True)
    # motion_seq = get_motion()
    # clear_motion()
    ...
    while (True):
        ...
        # elif action == Action.swing:
            ...
            swing(eef_pose_swing,[0,0,80],repeat=2,bilateral=True)
            LIFT_STOPPED = True
            # spin(vel=0.5)
            _, out = monitoring(1, filter=True)
            ft = np.mean(out, axis=0)
            ...
            if torque > 0:
                ...
                swing_param = [0, j4(sw_idx), -j5(sw_idx)]
            swing(eef_pose_swing,swing_param,repeat=2)
            # swing(eef_pose_swing,[50,0,0],repeat=8)
            N_swing += 1
        # else:
            # swing(eef_pose_lifted,[0,0,80],repeat=3,bilateral=True)
